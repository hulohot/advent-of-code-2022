...
def is_overlapping(tuple_a, tuple_b):
    """
    This function checks if tuple_a or tuple_b is overlapping
    with the other tuple.
    """
    start_inner, end_inner = tuple_a
    start_outer, end_outer = tuple_b
    return (
        start_inner <= start_outer <= end_inner
        or start_inner <= end_outer <= end_inner
        or start_outer <= start_inner <= end_outer
        or start_outer <= end_inner <= end_outer
    )
...
if __name__ == "__main__":
    ...
    for d in data.splitlines():
        ...
        if is_overlapping(data_list[0], data_list[1]):
            counter += 1
